...
fun FoodPlaces(navController: NavHostController, itemFromDBViewModel: ItemFromDBViewModel, userViewModel: UserViewModel) {
    key(navController.currentDestination?.route) {
        val isLoading by itemFromDBViewModel.isLoadingFood.observeAsState(initial = true)
        var places by remember { mutableStateOf(itemFromDBViewModel.getFoodPlaces()) }
        ...
        LaunchedEffect(isLoading) {
            if (isLoading) {
                ...
                places = itemFromDBViewModel.getFoodPlaces()
                isTimeout = false
            }
        }
